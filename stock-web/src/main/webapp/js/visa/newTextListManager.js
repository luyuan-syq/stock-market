			...
			.extend(
					SYQ.TextListManager.prototype,
					{
						...
						_searchData : function() {
							...
							if (countryId) {
								postData.countryId = $("#selectedCountry")
										.val();
							}
						...
						_removeGridRows : function(event) {
							...
							accountNamesKey = 'id';
							removeGridRowsURL = this.urls.deleteTextValueUrl;
									...
									.confirm({
										...
										onClose : function(success) {
											if (success) {
												 ...
												 * 'delete');
												 ******************************/
												data["fileId"] = principalName;
												data["countryId"] = $("#selectedCountry").val();
...
function loadTextListManager() {
	textListManager._buildGrid();
}

function addVisaFileTemplate() {
	var type = $("input[type=radio][name=visaFileTemplateType]:checked").val();
	if (type == 1) {
		var value = $("input[name=visaFileTemplateValue]").val();
		if ($.trim(value) == "") {
			alert("请填写非标准文件内容");
			return;
		}
		...
		$.ajax({
			url : "newVisa/addVisaTextListToCountry",
			type : 'POST',
			dataType : "json",
			data : postData,
			success : function(data) {
				...
				if (data.success) {
					SYQ.Console.showTip('添加非标准文件成功', 'success', 4000,
							new function() {
								textListManager._buildGrid();
							});
				} else {
					SYQ.Console.showTip(data.message, 'error', 5000);
		...
		var value = $("select[name=visaFileTemplateValue]").val();
		var postData = {
			countryId : countryId,
			fileId : value
		};
		$.ajax({
			url : "newVisa/addVisaFileTemplateToCountry",
			type : 'POST',
			dataType : "json",
			data : postData,
			success : function(data) {
				...
				if (data.success) {
					SYQ.Console.showTip('添加标准文件成功', 'success', 4000,
							new function() {
								textListManager._buildGrid();
							});
				} else {
