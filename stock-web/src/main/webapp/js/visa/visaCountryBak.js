	...
	$.extend(SYQ.VisaCountry.prototype,
			{
				...
				_buildDialog : function() {
					var self = this;
					var addAccountDialogTemp = this.$addCountryDialog
							.omDialog({
								title : '添加国家',
								height : '390',
								width : '520',
					...
					var editAccountDialogTemp = this.$editCountryDialog
							.omDialog({
								title : '编辑国家',
								height : '390',
								width : '520',
				...
				_submitAddCountry : function() {
					...
					var _$form = this.$addCountryDialog
							.find('form#country-info-form');
					...
					$.ajax({
						...
						success : function(data) {
							if (data.success) {
								...
								SYQ.Console.showTip('添加国家失败', 'info', 5000);
							}
				...
				_submitEditCountry : function() {
					...
					var _$form = this.$editCountryDialog
							.find('form#country-info-form');
					var _$formValidate = _$form.syqValidate({
						rules : {
							'code' : {
				...
				_deleteCountry : function(id) {
					...
					$.ajax({
						...
						success : function(data) {
							if (!data.success) {
								SYQ.Console.showTip('删除国家失败', 'info', 5000);
							}
	...
	$("#visaCountryTree").jstree(
						...
						return {
							...
							motifyUtsNode : {
								...
								action : function(NODE) {
									...
									visaCountry._openEditCountryDialog($(NODE)
											.attr("id"));
								}
							},
							deleteUtsNode : {
			...
			function(event, data) {
				if (data.rslt.obj.attr("icon") != undefined
						&& data.rslt.obj.attr("icon") != 'root') {
					var id = $(data.rslt.obj).attr("id");
					// $('#detail').omTabs('activate', 0);
					countryCode = $(data.rslt.obj).attr("code");
					countryId = $(data.rslt.obj).attr("id");
					$('#countryVisaDetail').omTabs('reload', 0,
							"./visaTemplate/template?code=" + countryCode);
					return;
				}
		...
		return false;
			...
			function(event, data) {
				...
				if (clickNode != undefined) {
				  $.jstree._reference('visaCountryTree').select_node($("#visaCountryTree ul ul li:first"), true, "");
				}
