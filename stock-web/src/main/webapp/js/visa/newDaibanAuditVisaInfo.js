var rejectFileFlowDialog ;
var cafileId;
var flowStatus;
var ENGINE_CONTEXT_PATH = "";
$(function() {
	...
	SYQ.TextListManager = function(options) {
		...
		ENGINE_CONTEXT_PATH = options.ENGINE_CONTEXT_PATH;
		this.urls = options.urls;

		this.fileTemplateGrid = $(".text-list-grid");
		this.grid_limit = 10;
		this.grid_height = 400;
		this.$rejectFileFlowDialog = $("#reject-file-flow-dialog");
			...
			.extend(
					SYQ.TextListManager.prototype,
					{
						...
						_buildDialog : function() {
							...
							var self = this;
							rejectFileFlowDialog = this.$rejectFileFlowDialog
									.omDialog({
										title : '拒绝',
										height : '390',
										width : '520',
						...
						_searchData : function() {
							var postData = {};
							postData.principalId = $("#visaPrincipalId").val();
							postData.flowStatusArray=1;
									...
									if (rowData.type == 0) {
										if (rowData.flowStatus == 1) {
											...
											menu += "<div class='edit' id='edit-account' type='edit' name='"
													...
													+ that.CONTEXT_PATH
													+ "fileTemplate/"
													+ rowData.flowStatusValue
													+ "' >下载</a></div>";
											menu += "<div class='add' id='add' type='add' name='"
													+ rowData.id 
													+ "' onclick='updatePassFlowStatus("
													+ rowData.id
													+ ",3)'>通过</div>";
											menu += "<div class='add' id='add' type='add' name='"
												+ rowData.id
												+ "' onclick='rejectFlowStatusDilog("
												+ rowData.id
												+ ",2)'>拒绝</div>";
										}
									}
...
function updateFlowStatus(id,status) {
	...
	$.ajax({
		url : './newVisa/updateVisaFileFlowStatus',
		type : 'POST',
		data : data,
		success : function(data) {
			if (data.success) {
				SYQ.Console.showTip('操作成功', 'info',
						5000);
				//window.location.href = ENGINE_CONTEXT_PATH + "passport/todoList?select=4"
				window.location.reload();

			} else {
				SYQ.Console.showTip('操作失败', 'info',
						5000);
			}
		}
	});
...
function updatePassFlowStatus(id,status) {

	var data = {};
	data.id = id;
	data.flowStatus = status;
	$.ajax({
		url : './newVisa/updatePassVisaFileFlowStatus',
		type : 'POST',
		data : data,
		success : function(data) {
			if (data.success) {
...
function rejectFlowStatusDilog(id,status) {
	$("#rejectFlowMsg").val("");
	cafileId = id;
	flowStatus = status;
	rejectFileFlowDialog
...
function rejectFlowStatus() {
	
	var data = {};
	data.id = cafileId;
	data.flowStatus = flowStatus;
	data.flowMsg = $("#rejectFlowMsg").val();
	$.ajax({
		url : './newVisa/updateRejectVisaFileFlowStatus',
		type : 'POST',
		data : data,
		success : function(data) {
			if (data.success) {
