(function() {
	...
	SYQ.Role = function(options) {
		...
		this.$appAdd = $(".tab-app");
		this.managerListGrid = $(".manager-grid");
		this.grid_limit = 10;
		this.grid_height = 400;
		// dialog 使用了omDialog, dom结构会被移动
		this.$addRoleDialog = $('#create-role-dialog');
			...
			.extend(
					SYQ.Role.prototype,
					{
						...
						_buildDialog : function() {
							var self = this;
							var addAccountDialogTemp = this.$addRoleDialog
									.omDialog({
										title : '添加角色',
										height : '390',
										width : '520',
						...
						_buildGrid : function() {
							...
							return $el.omGrid({
								...
								dataSource : this.urls.getManagerURL,
						...
						_renderBtnInGrid : function($el, single) {
							...
							var bindBtnEvent = function() {
								...
								menu.find(".edit").click(function(event) {
									...
									window.location.href = "role/editBeforeUrl?id="+$(source).attr('name');
									menu.hide();
									event.stopPropagation();
								});
							...
							var actionColModel = {
									...
									renderer : function(colValue, rowData, rowIndex) {
										var menu = "<div id='op-menu-" + rowIndex
												+ "' class='aps-op-menu'>";
										menu += "<div class='edit' id='edit-manager' type='edit' name='"
												+ rowData.id + "'>编辑</div>";
						 ...
						 */
						_openCreateRoleDialog : function(event) {
							var self = this;
							var data = null;
							var actionType = '';
							var source = null;
							var url = this.urls.addRoleBeforeURL;
							if (null == event) {
								actionType = 'create';
							} else {
								url = this.urls.editRoleBeforeURL;
								actionType = 'edit';
								source = $(event.currentTarget);
								if (source.is("span")) {
									source = $(event.target);
						...
						_createRoleDialog : function(html, actionType) {
							this.$addRoleDialog.html(html);
							var self = this;
							var _$form = this.$addRoleDialog
									.find('form#role-info-form');
							...
							var addRoleDialogTemp = this.$addRoleDialog
									.omDialog('open');
							_$form.submit(function() {
